...
$Commands{&commandRegexp( "show", "drvolume" )} = sub {
  ...
  my @query = &runTabdelDsmadmc("select state, count(volume_name) from drmedia group by state");
  #my @query = &runTabdelDsmadmc("select location, count(volume_name) from drmedia where location<>'NULL'  group by location");
  return if ( $#query < 0 );

  &setSimpleTXTOutput();

  &universalTextPrinter( "DR State\t# of Volumes", @query );

  return 0;

};
&defineAlias( 'sh drvolumes', 'sh drvolume' );
...
#####################
# Show DirSpace
# Create a device class entry for each directory defined to in the file devclass dedup storage pool.
# 	define devclass <devclassName> devtype=file directory=<directory> 
#  
# Reached out to Michelle to understand why the math is incorrect. 
#####################
&msg( '0110D', 'SHow dirspace' );
$Commands{qr/^(show|^sho|^sh)\s*(dirspace)(\S*)/i} = sub {
	
	my @content = &runTabdelDsmadmc("q dirspace");
	#my @content = &runTabdelDsmadmc("q dirspace file");
 		

 my $i=0;
 my @totals;	 
# my @content = ("ACTIVELOG        /tsm/activelog/aixtsm1                        64,424,509,440  B      36,549 M",
#							  "ACTLOG_MIR       /tsm/activelogmirror/aixtsm1                   62,914,560   K      36,549 M",
# 								"ARCHFAILO        /tsm/archlogfailover/aixtsm1                  61,376 M      61,126 M",
# 								"ARCHIVELOG       /tsm/archlog/aixtsm1                          60 G      55,647 M"); 						
#								
 my @printable;
  ...
  foreach ( @content ) {
     ...
     #############################
     #Converts Estimated to Bytes
     #############################
      if ( $line[3] eq "B"){
      	$line[2] =~ s/,//g;
      	$line[6] = ($line[2]);
      	$totalCapacity+=$line[6];
      	$line[7] = &byteFormatter ( $line[6], 'B' );	
      }
      #Need to validate how the units appear
      elsif ($line[3] eq "K"){
      	$line[2] =~ s/,//g; 
				$line[6] = ($line[2] * 1024 );
      	$totalCapacity+=$line[6];
      	...
      	$line[2] =~ s/,//g;
      	$line[6] = ($line[2] * 1024 * 1024 );
      	$totalCapacity+=$line[6];
      	$line[7] = &byteFormatter ( $line[6], 'B' );	
      }
      elsif ($line[3] eq "G"){
      	$line[2] =~ s/,//g;
      	$line[6] = ($line[2] * 1024 * 1024 * 1024);
      	$totalCapacity+=$line[6];
      	$line[7] = &byteFormatter ( $line[6], 'B' );	
      }
      
      #########################################
      #Converts Estimated Available to Bytes
      #########################################
      if ( $line[5] eq "B"){
      	$line[4] =~ s/,//g;
      	$line[8] = ($line[4]);
      	$availableCapacity+=$line[8];
      	$line[9] = &byteFormatter ( $line[8], 'B' );	
      }
      #Need to validate how the units appear
      elsif ($line[5] eq "K"){
      	$line[4] =~ s/,//g; 
      	...
      	#print "line4 is ", $line[4], "\t\n"; 
      	#Subtoutine %s redefined
      	$line[9] = convertz($line[4], $line[5] );   
      #	print "line9 is ", $line[9], "\t\n";
      	#$line[4] =~ s/,//g;
      	#$line[8] = ($line[4] * 1024 * 1024 );
      	#$availableCapacity+=$line[8];
      	#$line[9] = &byteFormatter ( $line[8], 'B' );	
      }
      elsif ($line[5] eq "G"){
      	$line[4] =~ s/,//g;
      	$line[8] = ($line[4] * 1024 * 1024 * 1024);
      	$availableCapacity+=$line[8];
      	$line[9] = &byteFormatter ( $line[8], 'B' );	
      }
      
      push( @printable, join( "\t", $line[0], $line[1], $line[7], $line[9]  )); 
 
 }
      push ( @printable, "\n");
      push ( @printable, "\n");

	push ( @totals, join("\t",  &byteFormatter ( $totalCapacity,'B' ), &byteFormatter ( $availableCapacity,'B' ) ) );

	
    
		
  &universalTextPrinter("Device Class\tDirectory\tVolume Capacity\tAvailable Capacity", @printable);
  

	&universalTextPrinter("Total Capacity\tAvailable Capacity", @totals);
  ...
  sub convertz{
  	...
  	if ( $Units eq "B"){                                                                                                         
		...
		#Need to validate how the units appear                                    
			...
			$b_Bytes = ($bb * 1024 * 1024 );                                                       
		}                                                                         
		elsif ($Units eq "G"){                                                                                                      
			$b_Bytes = ($bb * 1024 * 1024 * 1024);
		}                                                                         

		$availableCapacity+=$b_Bytes;          
		
		return &byteFormatter( $b_Bytes, 'B' );
  }
    1;
...
$Commands{qr/^(show)\s*(Expp)(\S*)/i} = sub {
  ...
  foreach ( @content ) {
      ...
      $line[3] = &timeFormatter ( $line[3], 'M' );
      
      if ( $line[7] ne "YES" ) {
         $line[7] = &colorString( "$line[7]", "BOLD RED" );
        }
        
        $line[5] = sprintf( "%3d", $line[5] );
        
        while ($line[5] =~ s/^(-?\d+)(\d\d\d)/$1,$2/) {
        	1;
       	}
       	
       	while ($line[6] =~ s/^(-?\d+)(\d\d\d)/$1,$2/) {
        	1;
       	}
      ...
      &universalTextPrinter("Date\tStart Time\tEnd Time\tDuration\tThreads\t{RIGHT}#Objects\t{RIGHT}Object/Min\tStatus",@printable);
  return 0;
};
...
$Commands{&commandRegexp( "show", "volreclaim" )} = sub {
  ...
  my @query = &runTabdelDsmadmc("select volume_name, stgpool_name,int(((EST_CAPACITY_MB*PCT_UTILIZED)/100))/1024,EST_CAPACITY_MB, PCT_UTILIZED, pct_reclaim from volumes where pct_reclaim>=60 and stgpool_name like upper('%".$3."%') order by 5 desc");
  return if ( $#query < 0 );

  &setSimpleTXTOutput();

  &universalTextPrinter( "#{%3s}\tVolumes Name (60%)\tStgPool\tActual Utilization(GB)\tEST Capacity\t%PCT Utilized\tPCT Reclaim", &addLineNumbers( @query ) );
  return 0;
...
&msg( '0110D', 'SHow stgpooldir' );
$Commands{&commandRegexp( "show", "stgpooldir" )} = sub {
  ...
  my @query = &runTabdelDsmadmc("select DIR_NAME, FREE_SPACE_MB, TOTAL_SPACE_MB, ACCESS from STGPOOL_DIRS");
  #my @query = &runTabdelDsmadmc("select volume_name, stgpool_name,int(((EST_CAPACITY_MB*PCT_UTILIZED)/100))/1024,EST_CAPACITY_MB, PCT_UTILIZED, pct_reclaim from volumes where pct_reclaim>=60 and stgpool_name like upper('%".$3."%') order by 5 desc");
  return if ( $#query < 0 );

  &setSimpleTXTOutput();

  &universalTextPrinter( "#{%3s}\tStgPoolDir\tFreeSpace_MB\tTotalSpace_MB\tAcess", &addLineNumbers( @query ) );
  return 0;
